...
class DEN():
	...
	def restore_params(self, task_id, trainable=True, param_values=dict()):
		self.params = dict()
		for scope_name, value in param_values.items():
			scope_name = scope_name.split(':')[0]
			[scope, name] = scope_name.split('/')
			
			if ('l%d' % den_layers in scope) and (task_id in name): 
				trainable = True
			elif ('l%d' % den_layers in scope):
				trainable = False
	...
	def add_output(task_id, output_len):

		self.initialize_parameters(output_len=output_len)

		prev_dim = self.w_fc.get_shape().as_list()[0]
		next_dim = self.w_fc.get_shape().as_list()[1]
		w_new_out = self.get_variable(name="w_"+str(task_id), shape=[prev_dim, 1], scope="l"+str(den_layers))
		b_new_out = self.get_variable(name="b_"+str(task_id), shape=[1], scope="l"+str(den_layers))
		
		self.w_fc = tf.concat([self.w_fc, w_new_out],1)
		self.b_fc = tf.concat([self.b_fc, b_new_out],0)
	...
	def optimization(self, task_id, y_conv, selection=False):
		...
		loss = tf.reduce_mean(loss + self.l2_mu * l2_regular)
...
# show image using cv
	...
	while den.last_label_index != (len(label_names) - 1):     # Loop for adding new tasks (lifelong learning)
		...
		if task_id == 1:
			...
			y_conv = den.add_output(task_id=task_id, output_len=den.last_label_index+1)		# Adding new unit in output layer for the new task
			
			print("--------Performing Selective Retraining---------")
			
			# ---------Selection-----------
			den.optimization(task_id, y_conv=y_conv, selection=True)
			den.train_task(task_id, y_conv, batch_size, epochs)		

			# ---------Retraining-----------

			_param = den.get_params()              # Values after hidden-outer training
			den.destroy_graph()
			den.restore_params(task_id=task_id, param_values=_param)
			...
			if result == False:
				...
				den.destroy_graph()
				den.restore_params(task_id=task_id, param_values=_param)        # Restoring model weights that were trained upto prev task
				...
				if result == False:
					...
					den.destroy_graph()
					den.restore_params(task_id=task_id, param_values=param_values)        # Restoring model weights that were trained upto prev task
...
# 				 = tf.nn.relu(tf.matmul(self.h_pool2_flat, w_fc) + b_fc)
