...
class DEN():
	...
	def build_model(self, task_id, x, keep_prob, retraining=False, expansion=False, splitting=False):
		...
		if task_id == 1:
			...
			W_conv1 = self.weight_variable(task_id, [5, 5, 3, 32])
			b_conv1 = self.bias_variable(task_id, [32])
			...
			W_conv2 = self.weight_variable(task_id, [5, 5, 32, 64])
			b_conv2 = self.bias_variable(task_id, [64])
			...
			W_fc1 = self.weight_variable(task_id, [15 * 15 * 64, 1024])
			b_fc1 = self.bias_variable(task_id, [1024])
			...
			#second fc layer
			W_fc2 = self.weight_variable(task_id, [1024, 128],"den_layer")
			b_fc2 = self.bias_variable(task_id, [128],"den_layer")

			h_fc2 = tf.matmul(h_fc1_drop, W_fc2) + b_fc2

			# readout fc layer
			W_fc3 = self.weight_variable(task_id, [128, 2])
			b_fc3 = self.bias_variable(task_id, [2])
...
# show image using cv
	...
	for task_id in range(1, len(label_names)):
		...
		[task_train, task_train_labels, task_test, task_test_labels] = den.add_task(task_id, label_names)			
